$(document).ready(function() {
  ...
  _h = $('head');
  var mc = metroOptions.getOption('color'),
    $colors = metroOptions.getColors(),
    $color = $colors[mc];
  if (mc !== 'default') {
    ...
    $lighter = [
      'hsl(' + hsl[0],
      hsl[1] * 100 + '%',
      Math.floor(hsl[2] * 130) + '%)'
    ].join(',');
    var html = ('.news .nerdz_date, .news .post_icons, a, .spoiler > span, .question, #profilePostArrow, #projectPostArrow {' + '  color: %color%;' + '}' + '.active {' + '  color: %color% !important; ' + '}' + '.news, .img_frame, .yt_frame, .window, .caption  {' + '  border-color: %color% !important;' + '}' + '.img_frame:after, .yt_frame:after {' + '  border-top: 28px solid %color%;' + '}' + '.window > .caption {' + '  background-color: %color%;' + '}' + '.sidebar li a:hover {' + '  background-color: %lighter% !important;' + '}' + '.more' + '{' + '  box-shadow: 0px -10px 10px 0px %color%;' + '}' + '.link:hover, a:hover {' + ' color: %lighter%;' + '}').replace(/\%color\%/g, $color).replace(/\%lighter\%/g, $lighter);
    $('<style type="text/css">').html(html).appendTo(_h);
    $('body').addClass(mc);
  }
  for (var col in $colors)
    $('<div>').attr('class', 'tile tiny').html('<div class="tile-content icon"><i class="icon-record" style="color:' + $colors[col] + ';"></i></div>').attr('title', col).addClass(mc === col ? 'selected' : '').appendTo($('#color-switcher'));
  var ts = $('#theme-switcher'),
    th = metroOptions.getOption('theme');
  ts.children('#' + th).addClass('selected');
  $('body').addClass(th);
  if (th === 'light') {
  ...
  $('#code-theme-switcher').on('change', function(e) {
    metroOptions.setOption('codeTheme', $('#code-theme-switcher').is(':checked') ? 'light' : 'dark');
    setTimeout(function() {
      location.reload();
    }, 1000);
  });
  var append_theme = '';
  if (metroOptions.getOption('codeTheme') === 'dark') {
    append_theme = '?skin=sons-of-obsidian';
    $('body').addClass('has-dark-theme');
  } else
    $('#code-theme-switcher').attr('checked', true);
  ...
  ts.on('click', '.tile', function() {
    if ($(this).hasClass('selected'))
      return false;
    metroOptions.setOption('theme', $(this).attr('id'));
    setTimeout(function() {
      location.reload();
    }, 1000);
  });
  $('#color-switcher').on('click', '.tile', function() {
    if ($(this).hasClass('selected'))
      return false;
    metroOptions.setOption('color', $(this).attr('title'));
    setTimeout(function() {
      location.reload();
    }, 1000);
  });
  tp = $('#metro-tagpanel');
  if (!metroOptions.getOption('tagPanel'))
    tp.attr('checked', false);
  tp.on('change', function(e) {
    metroOptions.setOption('tagpanel', tp.is(':checked'));
    TPUpdate();
  });
  tc = $('#tagpanel-custom');
  ...
  ns = $('#metro-notify');
  if (!metroOptions.getOption('notify'))
  ...
  plist.on('click', '.icon-pencil', function(e) {
    ...
    var form = function(fid, hpid, message) {
      return '<form style="margin-bottom:40px" id="' + fid + '" data-hpid="' + hpid + '">' + '<textarea class="row thin" id="' + fid + 'abc" autofocus style="width:100%; height:125px">' + message + '</textarea>' + '<div class="row thin"><span class="span3 place-right">' + '<input type="submit" class="place-right" value="' + N.getLangData().EDIT + '" style="width:45%" />' + '<button type="button" class="preview" data-refto="#' + fid + 'abc">' + N.getLangData().PREVIEW + '</button></span>' + '<input type="button" class="smallest span1 notag no-margin" onclick="window.open(\'/bbcode.php\')" value="BBCode" style="display: none;">' + '<button type="button no-margin" class="cancel">' + N.getLangData().CANCEL + '</button>' + '</div></form>';
    };
  ...
  plist.on('click', '.icon-eye-2', function() {
    //unlurk
    var me = $(this);
    var tog = function(d) {
      if (d.status === 'ok') {
  ...
  plist.on('click', '.icon-eye', function() {
    //lurk
    var me = $(this);
    var tog = function(d) {
      if (d.status === 'ok') {
  ...
  setInterval(function() {
    ...
    if (!isNaN(val) && val !== curpm && metroOptions.getOption('notify')) {
      nw = val - curpm;
      if (nw > 0) {

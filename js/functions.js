...
if (!String.prototype.autoLink) {
  String.prototype.autoLink = function() {
    if (localStorage.getItem('no-autolink') !== undefined)
      return this;
    str = this;
  ...
  if (!localStorage.getItem('no-tagpanel')) {
    ...
    for (i = 0; i < $l.length; ++i)
      if ($l.charAt(i) === '1')
        ...
        ch.eq(i).hide();
    if (tp.children('button').position().left > tp.width() - 50)
      $('.seeall').show();
    else
      $('.seeall').hide();
  } else {
...
TPLoad = function() {
  ...
  tp.on('click', 'a', function(e) {
    ...
    el.insertAtCaret(n);
    if (content === '' && tag.indexOf('/') > -1)
      el.setCursorPosition(p + Math.floor(n.length / 2));
    return false;
    ...
    autogrow: function() {
      ...
      return this.each(function() {
        ...
        var update = function(forced) {
          ...
          var twinContent = copy.html().replace(/<br>/gi, '<br />');
        ...
        $textarea.bind('blur', function() {
          if (copy.height() < maxheight)
            $textarea.height(copy.height() > minheight ? copy.height() : minheight);
        });
...
var metroOptions = new function() {
    ...
    this.setOption = function(option, value) {
      ...
      return localStorage.setItem('metro-options', JSON.stringify(this.getOptions()));
    };
